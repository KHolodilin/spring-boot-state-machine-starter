...
public final class StateMachineSchemaManager {
    ...
    private void validate() {
        ...
        requireColumns("state_machine_instance",
                Set.of("machine_type", "machine_id", "state", "context", "version", "created_at", "updated_at"),
                errors);
        requireColumns("state_machine_event",
                Set.of("event_id", "machine_type", "machine_id", "event_type", "from_state", "to_state", "result", "created_at"),
                errors);
        requireColumns("state_machine_request",
                Set.of("id", "event_id", "machine_type", "machine_id", "event_type", "payload", "status",
                        "retry_count", "locked_by", "locked_until", "created_at", "processed_at"),
                errors);
    ...
    private void requireColumns(String table, Set<String> required, List<String> errors) {
        List<String> columns = jdbcTemplate.query(
                ...
                WHERE table_schema = current_schema() AND table_name = ?
                """,
                (rs, rowNum) -> rs.getString(1).toLowerCase(Locale.ROOT),
                table);
    ...
    private void requirePrimaryKey(String table, List<String> columns, List<String> errors) {
        List<String> pk = jdbcTemplate.query(
                """
