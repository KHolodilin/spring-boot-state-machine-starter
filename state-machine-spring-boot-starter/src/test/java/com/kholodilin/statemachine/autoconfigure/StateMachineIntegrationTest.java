...
@SpringBootTest(
        ...
        properties = {
                ...
                "state-machine.async.recovery.interval=1s"
        })
@Testcontainers
class StateMachineIntegrationTest {
    ...
    enum OrderState { NEW, PAYMENT_PENDING, COMPLETED, CANCELLED }

    enum OrderEvent { START, PAYMENT_RESERVED, PAYMENT_REJECTED }
    ...
    void sendSuccessDuplicateAndRejected() {
        TransitionResult first = service.send(
                "order-saga",
                new StateMachineEvent<>("evt-start", "order-1", OrderEvent.START, null));
        ...
        TransitionResult duplicate = service.send(
                "order-saga",
                new StateMachineEvent<>("evt-start", "order-1", OrderEvent.START, null));
        ...
        TransitionResult reserved = service.send(
                "order-saga",
                new StateMachineEvent<>("evt-pay", "order-1", OrderEvent.PAYMENT_RESERVED, null));
        assertThat(reserved.outcome()).isEqualTo(TransitionOutcome.SUCCESS);

        TransitionResult rejected = service.send(
                "order-saga",
                new StateMachineEvent<>("evt-late", "order-1", OrderEvent.START, null));
        ...
        TransitionResult rejectedAgain = service.send(
                "order-saga",
                new StateMachineEvent<>("evt-late", "order-1", OrderEvent.START, null));
    ...
    void sendAsyncIsProcessedByWorker() {
        ...
        await().atMost(Duration.ofSeconds(10)).untilAsserted(() -> {
            ...
            assertThat(store.find("order-saga", "order-async").orElseThrow().state()).isEqualTo("PAYMENT_PENDING");
    ...
    void recoveryRequeuesLostMemoryItem() {
        var submission = service.sendAsync(
                "order-saga",
                new StateMachineEvent<>("evt-rec", "order-rec", OrderEvent.START, null));
        await().atMost(Duration.ofSeconds(10)).untilAsserted(() ->
                assertThat(requestStore.findById(submission.requestId()).orElseThrow().status())
                        .isEqualTo(StateMachineRequest.DONE));
